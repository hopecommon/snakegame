...
void Game::createRamdonFood()
{
     ...
     */
    std::vector<SnakeBody> whitespace;
    for (int i = 1; i < this->mGameBoardWidth - 1; i++)
    {
        for (int j = 1; j < this->mGameBoardHeight - 1; j++)
        {
            if (this->mPtrSnake->isPartOfSnake(i, j))
                continue;
            else
                whitespace.push_back(SnakeBody(i, j));
        }
    }
    int rand_index = std::rand() % whitespace.size();
    this->mFood = whitespace[rand_index];
}
