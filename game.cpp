...
void Game::initializeGame()
{
    ...
    this->mMap = this->mPtrSnake->getMap();
    this->mDifficulty = 0;
    this->mPoints = 0;
...
void Game::createRamdonFood()
{
    ...
    for (int i = 1; i < this->mGameBoardHeight - 1; i ++)
    {
        ...
        {
            if(this->mPtrSnake->isPartOfSnake(j, i) || this->mPtrSnake->isOneOfBlocks(j, i))
            {
                continue;
            }
            else
...
{
    int number_of_barrier = this->mMap.size();
    for (int i = 0; i < number_of_barrier; i ++)
    {
        mvwaddch(this->mWindows[1], this->mMap[i].getY(), this->mMap[i].getX(), this->mMapSymbol);
    }
    wrefresh(this->mWindows[1]);
}
